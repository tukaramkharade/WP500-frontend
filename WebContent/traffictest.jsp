		...
		success : function(data) {
			...
			if(roleValue == 'Admin' || roleValue == 'ADMIN'){
				...
				var row = $("<tr>").append($("<td>").text(name),
						...
						$(".name_lan0").val(trafficrules.name);
			            $(".protocol_lan0").val(trafficrules.protocol);
			            $(".type_lan0").val(trafficrules.type);
			            $(".ip_addr_lan0").val(trafficrules.ipAddress);
			            $(".macAddress_lan0").val(trafficrules.macAddress);
			            $(".portNumber_lan0").val(trafficrules.portNum);
			            $(".action_lan0").val(trafficrules.action);
	 ...
	 function addRow() {
		 var newRow = $('<tr>');
	   	 ...
	   	 var cell2 = $('<td>');
		   	...
		   	// Adding 'tcp' option
		   	var optionTcp = $('<option>', {
		   	    value: 'TCP',
		   	    text: 'TCP'
		   	});
		   	selectProtocol.append(optionTcp);

		   	// Adding 'udp' option
		   	var optionUdp = $('<option>', {
		   	    value: 'UDP',
		   	    text: 'UDP'
		   	});
		   	selectProtocol.append(optionUdp);

		   	// Default selected option as 'tcp'
		   	selectProtocol.val('TCP');

		   	cell2.append(selectProtocol);
		   	...
		   	selectProtocol.append(optionIP);

		   	// Adding 'udp' option
		   	var optionMac = $('<option>', {
		   	    value: 'MAC',
		   	    text: 'MAC'
		   	});
		   	selectProtocol.append(optionMac);

		   	// Default selected option as 'tcp'
		   	selectProtocol.val('IP');

		   	cell3.append(selectProtocol);

		   	
	   	    
			var cell4 = $('<td style="width: 100px;">');
	   	    var input = $('<input>', {
	   	        type: 'text',
	   	        class: 'ip_addr_lan0',
	   	        name: 'ip_addr_lan0',
	   	 ...
	   	 var cell5 = $('<td style="width: 100px;">');
	   	    var input = $('<input>', {
	   	        type: 'text',
	   	        class: 'macAddress_lan0',
	   	        name: 'macAddress_lan0',
	   	        style: 'height: 10px; width: 200px;'
	   	    });
	   	    cell5.append(input);
	   	    
		   	
	   	 var cell6 = $('<td style="width: 100px;">');
	   	    var input = $('<input>', {
	   	        type: 'text',
	   	        class: 'portNumber_lan0',
	   	        name: 'portNumber_lan0',
	   	        style: 'height: 10px; width: 200px;'
	   	    });
	   	    cell6.append(input);
	   	...
	   	// Adding 'tcp' option
	   	var optionAccept = $('<option>', {
	   	    value: 'ACCEPT',
	   	    text: 'ACCEPT'
	   	});
	   	selectProtocol.append(optionAccept);

	   	// Adding 'udp' option
	   	var optionReject = $('<option>', {
	   	    value: 'REJECT',
	   	    text: 'REJECT'
	   	});
	   	selectProtocol.append(optionReject);

	   	// Default selected option as 'tcp'
	   	selectProtocol.val('ACCEPT');

	   	cell7.append(selectProtocol);
	   	
	   	var cell8 = $('<td>');
	   	
	    var hiddenInput = $('<input>', {
	        type: 'hidden',
	        class: 'operation_action_lan0',
	        name: 'operation_action_lan0',
	        value: '' // Initialize with an empty value
	    });
	    
	    var ackButton = $(
		'<button data-toggle="tooltip" class="addBtn" data-placement="top" title="Save" style="color: #35449a;">')
		.html('<i class="fas fa-check"></i>')
		.click(function() {
			addTrafficRulesLan0();
			});
	    
	    var editButton = $('<button data-toggle="tooltip" class="editBtn" data-placement="top" title="Edit" style="color: #35449a;">')
	        .html('<i class="fas fa-edit"></i>')
	        .click(function () {
	            // Retrieve the values from the row and perform the edit action
	            var trafficrules = {
	                name: cell1.find('.name_lan0').val(),
	                protocol: cell2.find('.protocol_lan0').val(),
	                type: cell3.find('.type_lan0').val(),
	                ipAddress: cell4.find('.ip_addr_lan0').val(),
	                macAddress: cell5.find('.macAddress_lan0').val(),
	                portNum: cell6.find('.portNumber_lan0').val(),
	                action: cell7.find('.action_lan0').val()
	    ...
	    var deleteButton = $('<button data-toggle="tooltip" class="delBtn" data-placement="top" title="Delete" style="color: red">')
	        .html('<i class="fas fa-trash-alt"></i>')
	        .click(function () {
	            // Retrieve the values from the row and perform the delete action
	            var trafficrules = {
	                name: cell1.find('.name_lan0').val(),
		...
		cell8.append(hiddenInput);
		cell8.append(ackButton);
	    cell8.append(editButton);
	    cell8.append(deleteButton);
	   	    ...
	   	    var table = $('.trafficRulesListTable_lan0');
...
<!-- 							<div class="form-container">
								<form id="trafficRulesForm_lan0">
									...
									<table class="bordered-table" style="margin-top: -1px;">
										...
										<tr>
											<td>Name</td>
											<td><input type="text" id="name_lan0" name="name_lan0" class="name_lan0"
												maxlength="31" style="height: 10px;" />
												</td>
										
											<td>Type</td>
											<td><select class="type_lan0" id="type_lan0" name="type_lan0"
												style="height: 33px;">
							...
							<div class="table-container">
								<table id="trafficRulesListTable_lan0" class="trafficRulesListTable_lan0"
